...
{
	public class GuestControllerClient : IDisposable, IGuestControllerClient
	{
			...
			QueueItem queueItem = CreateQueueItem("/client/{client-id}/guest/refresh-auth/{refresh-token}", HttpMethod.POST, new EmptyRequest(), GuestControllerAuthenticationType.None, delegate(GuestControllerResult<RefreshResponse> r)
			{
				...
				else
				{
					...
					else if (refreshGuestControllerTokenError is IRefreshProfileDisabledError || refreshGuestControllerTokenError is IRefreshTemporaryBanError)
					{
						...
						AccountBannedEventArgs e3 = new AccountBannedEventArgs();
					...
					else if (guestApiErrorCollection != null)
					{
						...
						AuthenticationUnavailableEventArgs e5 = new AuthenticationUnavailableEventArgs();
						this.OnAuthenticationLost(this, e5);
						failureCallback(r);
					}
					else if (data == null || data.token == null || data.token.access_token == null || data.token.refresh_token == null)
					{
						...
						AuthenticationUnavailableEventArgs e6 = new AuthenticationUnavailableEventArgs();
						this.OnAuthenticationLost(this, e6);
						failureCallback(r);
					}
					else
					{
		...
		public void RecoverPassword(RecoverRequest request, Action<GuestControllerResult<NotificationResponse>> callback)
		{
			QueueItem item = CreateQueueItem("/client/{client-id}/notification/recover-password", HttpMethod.POST, request, GuestControllerAuthenticationType.ApiKey, callback);
			queue.Add(item);
			ExecuteNextCall();
		}

		public void RecoverUsername(RecoverRequest request, Action<GuestControllerResult<NotificationResponse>> callback)
		{
			QueueItem item = CreateQueueItem("/client/{client-id}/notification/recover-username", HttpMethod.POST, request, GuestControllerAuthenticationType.ApiKey, callback);
			queue.Add(item);
			ExecuteNextCall();
		}

		public void ResolveMase(RecoverRequest request, Action<GuestControllerResult<NotificationResponse>> callback)
		{
			QueueItem item = CreateQueueItem("/client/{client-id}/notification/resolve-mase", HttpMethod.POST, request, GuestControllerAuthenticationType.ApiKey, callback);
			queue.Add(item);
			ExecuteNextCall();
		}

		public void UpgradeNrt(RecoverRequest request, Action<GuestControllerResult<NotificationResponse>> callback)
		{
			QueueItem item = CreateQueueItem("/client/{client-id}/notification/upgrade-nrt", HttpMethod.POST, request, GuestControllerAuthenticationType.ApiKey, callback);
			queue.Add(item);
			ExecuteNextCall();
		}
		...
		{
			QueueItem item = CreateQueueItem("/client/{client-id}/notification/email-verification", HttpMethod.POST, new EmptyRequest(), GuestControllerAuthenticationType.AccessToken, callback);
			queue.Add(item);
			ExecuteNextCall();
		}
			...
			wwwCall.OnDone += delegate
			{
				...
				else
				{
					...
					if (!string.IsNullOrEmpty(error))
					{
						...
						{
							string value = HttpLogBuilder.BuildTimeoutLog(requestId, item.Uri, item.HttpMethod, responseHeaders, responseText, timeToStartUpload, timeToFinishUpload, percentUploaded, timeToStartDownload, timeToFinishDownload, percentDownloaded, timeoutReason, timeoutTime);
							item.TimeoutLogs.Append(value);
					...
					{
						string message = HttpLogBuilder.BuildResponseLog(requestId, item.Uri, item.HttpMethod, responseHeaders, responseText, statusCode);
					...
					else
					{
						if (!string.IsNullOrEmpty(error))
						{
							...
							if (text2.Contains("connection appears to be offline") || text2.Contains("connection was lost") || text2.Contains("network is unreachable"))
							{
								logger.Critical(HttpLogBuilder.BuildResponseLog(requestId, item.Uri, item.HttpMethod, responseHeaders, responseText, statusCode));
